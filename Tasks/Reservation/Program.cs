...
namespace FirstProject
{
    class Program
    {
        ...
        {
            var zeitRaum = endDate.Date - startDate.Date;
            foreach (Reservation reservation in bookedReservations)
            {
                for (int i = 0; i < zeitRaum.Days; i++)
                {
                    if (startDate.Date.Add(new TimeSpan(i)) == reservation.From.Date || startDate.Date.Add(new TimeSpan(i)) == reservation.To)
                    {
                        return false;
                    }
